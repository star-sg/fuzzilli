...
class Binary {
  ...
  emit_type(type) {
    if ((typeof type) == 'number') {
      if (type >= 0) this.emit_u8(type);
      else this.emit_u8(type & kLeb128Mask);
...
class WasmModuleBuilder {
  ...
  static defaultFor(type) {
    if (type == kWasmI32) return wasmI32Const(0);
    if (type == kWasmI64) return wasmI64Const(0);
    if (type == kWasmF32) return wasmF32Const(0.0);
    if (type == kWasmF64) return wasmF64Const(0.0);
    if (type == kWasmS128) return [kSimdPrefix, kExprS128Const, ...(new Array(16).fill(0))];
    if (type == kWasmStringViewIter || type == kWasmStringViewWtf8 || type == kWasmStringViewWtf16) {
      ...
      binary.emit_section(kTypeSectionCode, section => {
        ...
        for (let i = 0; i < wasm.types.length; i++) {
          ...
          if (type.supertype != kNoSuperType) {
            if (type.is_final) {
              section.emit_u8(kWasmSubtypeFinalForm);
            } else {
                section.emit_u8(kWasmSubtypeForm);
            }
            // section.emit_u8(type.is_final ? kWasmSubtypeFinalForm : kWasmSubtypeForm);
      ...
      binary.emit_section(kImportSectionCode, section => {
        ...
        for (let imp of wasm.imports) {
          ...
          if (imp.kind == kExternalFunction) {
            ...
            let flags_1 = 0;
            if (imp.mutable) flags_1 = 1;
            let flags_2 = 0;
            if (imp.shared) flags_2 = 0b10;
            let flags = flags_1 | flags_2;
            ...
            let limits_byte_1 = 0;
            if (is_memory64) limits_byte_1 = 4;
            let limits_byte_2 = 0;
            if (is_shared) limits_byte_2 = 2;
            let limits_byte_3 = 0;
            if (has_max) limits_byte_3 = 1;
            let limits_byte = limits_byte_1 | limits_byte_2 | limits_byte_3;
            // let limits_byte = (is_memory64 ? 4 : 0) | (is_shared ? 2 : 0) | (has_max ? 1 : 0);
    ...
    if (wasm.tables.length > 0) {
      ...
      binary.emit_section(kTableSectionCode, section => {
        ...
        for (let table of wasm.tables) {
          ...
          let limits_byte_1 = 0;
          if (table.is_table64) limits_byte_1 = 4;
          let limits_byte_2 = 0;
          if (table.is_shared) limits_byte_2 = 2;
          let limits_byte_3 = 0;
          if (table.has_max) limits_byte_3 = 1;
          let limits_byte = limits_byte_1 | limits_byte_2 | limits_byte_3;
          // let limits_byte = (table.is_table64 ? 4 : 0) | (table.is_shared ? 2 : 0) | (table.has_max ? 1 : 0);
          section.emit_u8(limits_byte);
          let emit = val => {
              if (table.is_table64) {
                  section.emit_u64v(val);
              ...
              // table.is_table64 ? section.emit_u64v(val) : section.emit_u32v(val);
          }
    ...
    if (wasm.globals.length > 0) {
      ...
      binary.emit_section(kGlobalSectionCode, section => {
        ...
        for (let global of wasm.globals) {
          ...
          let flags_1 = 0;
          if (global.mutable) flags_1 = 1;
          let flags_2 = 0;
          if (global.shared) flags_2 = 0b10;
          let flags = flags_1 | flags_2;
          section.emit_u8(flags);
    ...
    if (wasm.element_segments.length > 0) {
      ...
      binary.emit_section(kElementSectionCode, section => {
        ...
        for (let segment of segments) {
          ...
          let shared_flag = 0;
          if (segment.is_shared) shared_flag = 0b1000;
    ...
    if (wasm.data_segments.length > 0) {
      ...
      binary.emit_section(kDataSectionCode, section => {
        ...
        for (let seg of wasm.data_segments) {
          let shared_flag = 0;
          if (seg.is_shared) shared_flag = 0b1000;
...
let [wasmBrOnCast, wasmBrOnCastFail] = (function() {
  ...
  function wasmBrOnCastImpl(labelIdx, sourceType, targetType, brOnFail) {
    ...
    let tmp = kExprBrOnCast;
    if (brOnFail) tmp = kExprBrOnCastFail;
    return [
      kGCPrefix, tmp,
...
function getOpcodeName(opcode) {
  let result = globalThis.kWasmOpcodeNames?.[opcode];
